...
function getEventInfo() {
    // title
    let title = document.body.getElementsByStyle({
        'fontSize': '40px'
    })[0].textContent;
    ...
    if (startDate != null) {
        link += '&dates=' + startDate.replace(/[:\-\.]/g, '') + '/' + endDate.replace(/[:\-\.]/g, '');
    }
    if (lieu != null) {
...
function waitTitleIsSet(callback, checkFrequencyInMs, timeoutInMs) {
    var startTimeInMs = Date.now();
    (function loopSearch() {
        if (document.title != "Notion – The all-in-one workspace for your notes, tasks, wikis, and databases.") {
            callback();
            return;
        }
        else {
...
waitForElementToDisplay(".notion-scroller>div.whenContentEditable", function () {
    waitTitleIsSet(function () {
        ...
        if (calendarLink != null) {
            ...
            if (document.querySelector(".whenContentEditable :nth-child(3) > div > :nth-child(2) > div > :nth-child(1) > div > div") != null) {
                document.querySelector(".whenContentEditable :nth-child(3) > div > :nth-child(2) > div > :nth-child(1) > div > div").insertBefore(addToCalendar, document.querySelector(".whenContentEditable :nth-child(3) > div > :nth-child(2) > div > :nth-child(1) > div > div > div:nth-child(1)"));
            }
        }
    }, 100, 9000);
}, 100, 9000);
...
chrome.runtime.onMessage.addListener((msg, sender, response) => {
    if ((msg.from === 'popup') && (msg.subject === 'eventInfo')) {
